...
io.sockets.on ('connection', function (socket) {
	...
	socket.on ('findRoom', function (name, level) {
		...
		if (!id) {	//CREATE ROOM
			...
			games.push ({'id': id, 'level': level, 'recycle': 0, 'stage': 1, 'p1Face': 0, 'p2Face': 0, 'p1Name': name, 'p2Name': '', 'p1': 0, 'p2': 0, 'p1Size': 1, 'p2Size': 1, 'p1Rage': 0, 'p2Rage': 0, 'tile': terrain});
			levelPop [level]++;
		}
		socket.join (id);
	...
	socket.on ('select', function (tileNum, room, team, upgrade, attacker) {
		if (room) {
			...
			if (games [roomId].stage == 4) {
				...
				if (team == 0) {
					if (games [roomId].tile [tileNum] == 0 && games [roomId].p1 >= 3) {		//CREATE 3
						games [roomId].tile [tileNum] = -3;
						games [roomId].p1Size++;
						games [roomId].p1-= 3;
					}
					else if (games [roomId].tile [tileNum] == -3 && games [roomId].p1 >= 1) {	//TRANSITION 4, 7, 11
					...
					else if (games [roomId].tile [tileNum] == 3 && games [roomId].p1 >= 1) {	//KILL 3
						...
						if (games [roomId].tile [attacker] == -7 || games [roomId].tile [attacker] == -8 || games [roomId].tile [attacker] == -9) {
							...
							if (games [roomId].p1 >= 2 && games [roomId].tile [tileNum] == 4) price2 = 2;
							...
							if (games [roomId].tile [tileNum] > 10 && games [roomId].tile [tileNum] < 20) {
								...
								games [roomId].p1 += games [roomId].tile [tileNum]-9;
							}
						}
						if (price > 0) {
							games [roomId].tile [tileNum]--;
							...
							//SUPPLY
							if (games [roomId].tile [tileNum] <= -11) calculateBranches (roomId, 0);
						}
					}
					io.sockets.in (room).emit ('tiles', games [roomId].tile, team, games [roomId].p1, games [roomId].p1Rage, terrainPoint);
				}
				if (team == 1) {
					if (games [roomId].tile [tileNum] == 0 && games [roomId].p2 >= 3) {		//CREATE 3
						games [roomId].tile [tileNum] = 3;
						games [roomId].p2Size++;
						games [roomId].p2 -= 3;
					}
					else if (games [roomId].tile [tileNum] == 3 && games [roomId].p2 >= 1) {	//TRANSITION 4, 7, 11
						if (upgrade == 0) games [roomId].tile [tileNum]++;
						else if (upgrade == 1) games [roomId].tile [tileNum] = 7;
						else if (upgrade == 2) {
							games [roomId].tile [tileNum] = 11;
					...
					else if (games [roomId].tile [tileNum] == -3 && games [roomId].p2 >= 1) {	//KILL 3
						games [roomId].tile [tileNum] = 0;
						games [roomId].p1Size--;
						games [roomId].p2--;
						games [roomId].p2Rage++;
						...
						if (games [roomId].tile [attacker] == 7 || games [roomId].tile [attacker] == 8 || games [roomId].tile [attacker] == 9) {
							...
							if (games [roomId].tile [tileNum] < -10) {
								games [roomId].tile [tileNum]++;
								games [roomId].p2 += games [roomId].tile [tileNum]*-1-9;
							}
						}
						if (price2 > 0) {
							games [roomId].tile [tileNum]++;
							...
							//SUPPLY
							if (games [roomId].tile [tileNum] >= 11 && games [roomId].tile [tileNum] < 20) calculateBranches (roomId, 1);
						}
					}
...
function calculateBranches (roomId, oppTeam) {
	...
	for (branch=0; branch<searchTiles.length; branch++) {
		...
		if (oppTeam == 0 && games [roomId].tile [searchTiles [branch]] > -11) {
			...
			if (games [roomId].tile [searchTiles [branch]] < -3) {
