...
io.sockets.on ('connection', function (socket) {
	...
	//CLIENT CHAT
	socket.on ('chat', function (room, team, text) {
		io.sockets.in (room).emit ('chatReceived', team, text);
	});
		...
		if (room) {
			...
			if (games [roomId].stage == 4) {
				if (team == 0) {
					...
					io.sockets.in (room).emit ('tiles', games [roomId].tile, team, games [roomId].p1, games [roomId].p1Rage);
				}
				if (team == 1) {
					...
					io.sockets.in (room).emit ('tiles', games [roomId].tile, team, games [roomId].p2, games [roomId].p2Rage);
				}
			}
