				...
				if (team == 0) {
					if (games [roomId].tile [tileNum] == 0 && games [roomId].p1 >= 1 && upgrade != 2) {		//CREATE 3
						...
						if (price > 0) {
							...
							if (games [roomId].tile [tileNum] == 6 || games [roomId].tile [tileNum] == 3)  games [roomId].tile [tileNum] = 0;
							if (games [roomId].tile [tileNum] == 10)  games [roomId].tile [tileNum] = 0;
							calculateBranches (roomId, 1);
						}
					}
...
function calculateBranches (roomId, oppTeam) {
	...
	for (branch=0; branch<searchTiles.length; branch++) {
		...
		var searchTile = Math.abs (games [roomId].tile [searchTiles [branch]]);
...
function update () {
	...
	for (w=0; w<games.length; w++) {
		...
		if (games [w].stage == 4) {
			...
			games [w].p1 += timeSinceLastFrame/2000 + games [w].p1Income/10000;
			games [w].p2 += timeSinceLastFrame/2000 + games [w].p2Income/10000;
