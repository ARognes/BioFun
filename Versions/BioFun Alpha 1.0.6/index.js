...
io.sockets.on ('connection', function (socket) {
	...
	socket.on ('findRoom', function (name) {
		...
		if (!id) {
			...
			games.push ({'id': id, 'recycle': 0, 'stage': 1, 'p1Face': 0, 'p2Face': 0, 'p1Name': name, 'p2Name': '', 'p1': 0, 'p2': 0, 'p1Size': 1, 'p2Size': 1, 'p1Rage': 0, 'p2Rage': 0, 'tile': terrain});
		}
		socket.join (id);
...
function update () {
	...
	for (w=0; w<games.length; w++) {
		...
		if (games [w].recycle >= 60000 && games [w].stage > 1) {
